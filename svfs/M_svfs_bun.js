...
async function readFileStream(file, start, len){
    ...
    while (true) {
        ...
        if (value) {
            ...
            if(chunk_start_pos > (start + len - 1) || chunk_end_pos < start){
                // console.log('没有进入截取范围或已经离开了 不做处理');
                continue; 
            }else if(chunk_start_pos <= start && chunk_end_pos >= (start + len -1)){
                // console.log('这个块覆盖了整个截取范围');
                ...
                // console.log('这个块在截取范围中，只是一小块 直接push入临时列表中');
                
                arr_list.push(value);
            }else if(chunk_start_pos < start && chunk_end_pos >= start ){
                // console.log('这个块前面有一部分没有在截取范围中，但是后一部分进入了');
                
                const slice_start_pos = start - chunk_start_pos;
                const res = value.slice(slice_start_pos); 
                arr_list.push(res);
            }else if(chunk_start_pos >= start && chunk_start_pos <= (start + len -1) && chunk_end_pos > (start + len -1) ){
                // console.log('这个块前面有一部分在截取范围中，但是后一部分离开了');
                const slice_end_pos =  (start+len) - chunk_start_pos;
    ...
    for (const u8arr of arr_list) {
        // console.log('当前索引：', cur_index, '数据长度：', u8arr.length, '总长度：', len); 
